...
//---------------------------------------------------------------------------------------
class Pods_XComGameState_Job extends XComGameState_BaseObject config(LWOTC_PodManager);
...
function Pods_JobTemplate GetMyTemplate()
{
    ...
    Template = Pods_JobTemplate(TemplateMgr.FindStrategyElementTemplate(TemplateName));
    return Template;
}

function String GetDebugString()
{
    return " ["$`PODMGR.MissionJobs[ID].FriendlyName $"] " $ String(TemplateName);
}
