...
//---------------------------------------------------------------------------------------
class Pods_XComGameState_Job_Flank extends Pods_XComGameState_Job_MoveToLocation;
...
function static Vector GetFlankingLocation(XComGameState_AIGroup Group)
{
    ...
    XComLocation = `PODMGR.GetLastKnownXComPosition();
...
function bool ShouldEngage(XComGameState_AIGroup Group, Pods_XComGameState_Manager PodMgr, Vector TargetLocation)
{
    local XComGameState_AIPlayerData AIPlayerData;
    local Pods_XComGameState_Job OtherJob;
    ...
    for (i = 0; i < PodMgr.ActiveJobs.Length; ++i)
    {
        ...
        OtherJob = Pods_XComGameState_Job(History.GetGameStateForObjectID(PodMgr.ActiveJobs[i].ObjectID));
