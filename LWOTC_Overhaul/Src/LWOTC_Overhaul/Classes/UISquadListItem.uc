...
var StateObjectReference SquadRef; // reference to a Squad_XComGameState
...
simulated function Update()
{
	...
	arrUnitStates = SquadState.Soldiers.GetSoldiers();
	...
	StartIndex = idx;
	arrUnitStates = SquadState.Soldiers.GetTempSoldiers();
...
simulated function int GetClassIconAlphaStatus(XComGameState_Unit UnitState, Squad_XComGameState SquadState)
{
	// Dim the icon if the squad is on-mission and this soldier isn't on the mission, regardless of their actual status.
	if (SquadState.IsDeployedOnMission() && !SquadState.Soldiers.IsSoldierOnMission(UnitState.GetReference()))
		return 30;
	...
	switch(UnitState.GetStatus()) 
	{
		...
		case eStatus_OnMission:
			...
			return 100; //`LWOUTPOSTMGR.IsUnitAHavenLiaison(UnitState.GetReference()) ? 50 : 100;
