...
class EvacZone_X2Ability_PlaceDelayed extends X2Ability config(LWOTC_EvacZone);
...
static function X2AbilityTemplate PlaceDelayedEvacZone()
{
    ...
    local EvacZone_X2Condition_PlaceDelayedPermitted CanPlaceEvacZoneCondition;
    ...
    Template.TargetingMethod = class'EvacZone_X2TargetingMethod';
    ...
    CanPlaceEvacZoneCondition = new class'EvacZone_X2Condition_PlaceDelayedPermitted';
...
simulated function XComGameState PlaceDelayedEvacZone_BuildGameState(XComGameStateContext Context)
{
    ...
    Delay = GetEvacDelay();
    class'EvacZone_XComGameState_EvacSpawner'.static.InitiateEvacZoneDeployment(Delay, SpawnLocation, NewGameState);
...
{
    local EvacZone_XComGameState_EvacSpawner EvacSpawner;
    ...
    EvacSpawner = EvacZone_XComGameState_EvacSpawner(`XCOMHISTORY.GetSingleGameStateObjectForClass(class'EvacZone_XComGameState_EvacSpawner', true));
    ...
    Value.i = default.DEFAULT_EVAC_PLACEMENT_DELAY[`CAMPAIGNDIFFICULTYSETTING];
    ...
    if (EvacDelayTuple.Data.Length != 1 || EvacDelayTuple.Data[0].Kind != XComLWTVInt)
    {
        ...
        Delay = default.DEFAULT_EVAC_PLACEMENT_DELAY[`CAMPAIGNDIFFICULTYSETTING];
    }
    else
    {
        Delay = Clamp(EvacDelayTuple.Data[0].i, default.MIN_EVAC_PLACEMENT_DELAY[`CAMPAIGNDIFFICULTYSETTING], default.MAX_EVAC_PLACEMENT_DELAY);
    }
