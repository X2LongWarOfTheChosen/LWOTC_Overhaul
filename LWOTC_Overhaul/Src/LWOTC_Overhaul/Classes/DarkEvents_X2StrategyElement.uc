...
function bool CanActivateHunterClass_LW (XComGameState_DarkEvent DarkEventState)
{
	...
	return  (ResistanceHQ.NumMonths >= (7 - `CAMPAIGNDIFFICULTYSETTING));
}
...
{
	`ACTIVITYMGR.AddDoomToRandomFacility(NewGameState, GetMinorBreakthroughDoom(), default.MinorBreakthroughDoomLabel);
}
...
{
	`ACTIVITYMGR.AddDoomToRandomFacility(NewGameState, GetMajorBreakthroughDoom(), default.MajorBreakthroughDoomLabel);
}
...
function ActivateAlienCypher_LW(XComGameState NewGameState, StateObjectReference InRef, optional bool bReactivate)
{
	...
	IntelScalar.Difficulty = `CAMPAIGNDIFFICULTYSETTING; // Set to the current difficulty
