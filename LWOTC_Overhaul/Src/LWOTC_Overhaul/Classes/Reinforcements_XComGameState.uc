...
function Initialize()
{
	...
	{
		if (string(default.MISSION_REINFORCEMENT_MODIFIERS[k].MissionType) == class'Utilities_LWOTC'.static.CurrentMissionType())
		{
...
function int GetQueueOffsetModifier()
{
	...
	if (`SQUADMGR.IsValidInfiltrationMission(MissionState.GetReference()))
	{
		...
		{
			if (InfiltrationQueueModifier.Infiltration > SquadState.InfiltrationState.CurrentInfiltration)
			{
...
function bool CanAddToBucket()
{
	...
	{
		if (class'Utilities_LWOTC'.static.CurrentMissionType() != "Defend_LW" &&
		    class'Utilities_LWOTC'.static.CurrentMissionType() != "SupplyConvoy_LW" &&
		    class'Utilities_LWOTC'.static.CurrentMissionType() != "Invasion_LW")
		return false;
	}
...
function int CheckForReinforcements()
{
	...
	if (CanAddToBucket())
	{
		...
		TmpValue = default.DIFFICULTY_MODIFIER[`CAMPAIGNDIFFICULTYSETTING];
		...
		CavalryTurn = ReinfRules.CavalryAbsoluteTurn + default.TURN_COUNT_TO_CAVALRY_ALL_CASES_MOD[`CAMPAIGNDIFFICULTYSETTING];
		CavalryOnWinTurn = ReinfRules.CavalryWinTurn + default.TURN_COUNT_FOR_CAVALRY_AFTER_VICTORY_MOD[`CAMPAIGNDIFFICULTYSETTING];
...
function float GetIncreaseFromDarkEvents()
{
    ...
    {
        Modifier = RAPID_RESPONSE_MODIFIER[`CAMPAIGNDIFFICULTYSETTING];
    }
