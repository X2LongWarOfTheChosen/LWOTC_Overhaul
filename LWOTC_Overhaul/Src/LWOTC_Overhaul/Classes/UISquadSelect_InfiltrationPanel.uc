...
simulated function Update(array<StateObjectReference> Soldiers)
{
	...
	TotalInfiltrationHours = class'Squad_Static_Infiltration_Helper'.static.GetHoursToFullInfiltration(Soldiers, MissionState.GetReference(), SquadSizeHours, CovertnessHours, LiberationHours);
	...
	BaseInfiltrationTime.SetText(GetDaysAndHoursString(Round(class'Squad_Static_Infiltration_Helper'.static.GetBaselineHoursToInfiltration(MissionState.GetReference()))));
	...
	else
	{
		...
		else if (TotalMissionHours > TotalInfiltrationHours * (class'Squad_XComGameState_Infiltration'.static.GetRequiredPctInfiltrationToLaunch(MissionState) / 100.0))
	...
	if (class'Squad_Static_Infiltration_Helper'.default.MissionsAffectedByLiberationStatus.Find (MissionState.GeneratedMission.Mission.MissionName) == -1)
	{
		LiberationText.Hide();
