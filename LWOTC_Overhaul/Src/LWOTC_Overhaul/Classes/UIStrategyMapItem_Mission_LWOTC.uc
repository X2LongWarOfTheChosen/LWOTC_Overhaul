...
class UIStrategyMapItem_Mission_LWOTC extends UIStrategyMapItem_Mission;
...
function UpdateFromGeoscapeEntity(const out XComGameState_GeoscapeEntity GeoscapeEntity)
{
	...
	{
		InfiltrationPct = int(InfiltratingSquad.InfiltrationState.CurrentInfiltration * 100.0);
...
{
	return `SQUADMGR.Squads.GetSquadOnMission(GeoscapeEntityRef);
}
...
simulated function SetProgressBarColor(float RemainingSeconds)
{
	...
	PercentIdealInfiltration = (RemainingSeconds / 3600.0) / class'Squad_Static_Infiltration_Helper'.static.GetBaselineHoursToInfiltration(GeoscapeEntityRef);
