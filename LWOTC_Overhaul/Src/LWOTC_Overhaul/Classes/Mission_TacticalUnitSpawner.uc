//---------------------------------------------------------------------------------------
//  FILE:    Mission_TacticalUnitSpawner
...
//--------------------------------------------------------------------------------------- 
class Mission_TacticalUnitSpawner extends Object config(LW_Overhaul);
...
static function SpawnUnitsForMission(XComGameState_MissionSite Mission)
{
	...
	if (class'Utilities_LWOTC'.static.CurrentMissionIsRetaliation())
	{
		if (Mission == None)
		{
...
static function LoadCivilianUnitsFromOutpost()//XComGameState_LWOutpost Outpost)
{
	...
	for (i = 0; i < RebelsOnMission.Length; ++i)
	{
		...
		class'Utilities_LWOTC'.static.AddRebelToMission(Outpost.Rebels[RebelsOnMission[i]].Unit, Outpost.GetReference(), TemplateName, SpawnTiles[i], eTeam_Neutral);
	}
	*/
}
...
static function LoadResistanceMECsFromOutpost()//XComGameState_LWOutpost Outpost, bool SpawnNearObjective)
{
	...
	for (i = 0; i < Outpost.ResistanceMecs.Length; ++i)
	{
		...
		UnitTile = RootTile;
		FoundTile = class'Utilities_LWOTC'.static.GetSpawnTileNearTile(UnitTile, 2, 4);
		...
		if (FoundTile)
		{
			...
			ProxyUnit = class'Utilities_LWOTC'.static.CreateProxyUnit(Unit, 'ResistanceMEC', true, NewGameState, 'ResistanceMecM1_Loadout');
								...
								optional bool SpawnAtObjective = false)
{
	...
	if (FoundTile)
	{
		...
		PlayerState = class'Utilities_LWOTC'.static.FindPlayer(Team);
...
static function bool GetTileForCivilian(int i, out TTile UnitTile)
{
	...
	for (j = 0; j < default.arrEncounterZoneWidths.Length; ++j)
	{
		...
		if (class'Utilities_LWOTC'.static.GetSpawnTileNearTile(UnitTile, 3, 9))
		{
			return true;
		}
	}
